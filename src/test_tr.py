...
EPOCHS = 20  # 训练的轮数，表示模型将遍历整个训练数据的次数
...
checkpoint.restore(tf.train.latest_checkpoint(
    checkpoint_dir))  # 从最新的检查点文件中恢复模型参数

checkpoint.discriminator.summary()  # 打印判别器模型的摘要信息

# 生成一个随机噪声向量，通过生成器生成样本，然后通过判别器进行判别
val = checkpoint.discriminator(checkpoint.generator(
    tf.random.normal([1, NOISE_DIM]), training=False))
